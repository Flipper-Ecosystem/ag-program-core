...
impl DexAdapter for MeteoraAdapter {
    ...
    fn execute_swap(
        ...
        // The last account after bin arrays is program id (second occurrence), which should not be counted as bin array
        // Use saturating_sub to prevent underflow when remaining_accounts_count == MIN_ACCOUNTS
        const MAX_BIN_ARRAYS: usize = 5;
        const BIN_ARRAYS_START: usize = 16; // Bin arrays start after first Program ID
        
        let bin_arrays_count = remaining_accounts_count
            .saturating_sub(MIN_ACCOUNTS)
            .saturating_sub(1) // Exclude last account (second Program ID)
            .min(MAX_BIN_ARRAYS) as u8;
        ...
        let mut accounts = vec![
            ...
            AccountMeta::new(adapter_accounts[10].key(), false),       // host_fee_account
            ...
            AccountMeta::new_readonly(adapter_accounts[13].key(), false), // memo_program
            AccountMeta::new_readonly(adapter_accounts[14].key(), false), // event_authority
        ...
        for i in 0..bin_arrays_count {
            accounts.push(AccountMeta::new(adapter_accounts[BIN_ARRAYS_START + i as usize].key(), false));
        }
        ...
        for i in 0..bin_arrays_count {
            account_infos.push(adapter_accounts[BIN_ARRAYS_START + i as usize].clone());
        }
    ...
    fn validate_accounts(
        ...
        let program = &adapter_accounts[15];
