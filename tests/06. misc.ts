...
describe("Flipper Swap Protocol - End to End Tests for Swaps and Limit Orders with WSOL", () => {
    ...
    async function fundTokenAccount(mint: PublicKey, account: PublicKey, amount: number | bigint) {
    ...
    before(async () => {
        ...
        await mintTo(provider.connection, wallet.payer, intermediateMint, userIntermediateTokenAccount, wallet.publicKey, 1_000_000_000_000);

        // Fund vaults
        await fundTokenAccount(sourceMint, inputVault, 1_000_000_000_000);
        await fundTokenAccount(intermediateMint, intermediateVault, 1_000_000_000_000);
    ...
    it("1. Simple single-hop swap with Raydium adapter", async () => {
        ...
        const inputPoolVault = sourceMint.toBuffer().compare(destinationMint.toBuffer()) < 0 ? raydiumTokenAVault : raydiumTokenBVault;
        const outputPoolVault = sourceMint.toBuffer().compare(destinationMint.toBuffer()) < 0 ? raydiumTokenBVault : raydiumTokenAVault;
