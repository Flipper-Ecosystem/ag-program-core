  ...
  it("1. Single-hop swap with Whirlpool adapter (with supplemental tick arrays)", async () => {
    ...
    const [supplementalTickArray0] = PublicKey.findProgramAddressSync(
      [
        Buffer.from("tick_array"),
        whirlpoolPoolState.toBuffer(),
        Buffer.from(new Int32Array([-200]).buffer),
      ],
      mockWhirlpoolProgramId
    );
    ...
    const [supplementalTickArray2] = PublicKey.findProgramAddressSync(
      [
        Buffer.from("tick_array"),
        whirlpoolPoolState.toBuffer(),
        Buffer.from(new Int32Array([300]).buffer),
      ],
      mockWhirlpoolProgramId
    );
    ...
    if (!poolExists) {
      ...
      // Verify balances
      const walletTokenAInfo = await getAccount(
        provider.connection,
        walletTokenA
      );
      const walletTokenBInfo = await getAccount(
        provider.connection,
        walletTokenB
      );
    ...
    const [whirlpoolPoolInfo] = PublicKey.findProgramAddressSync(
      [
        ...
        getSwapTypeBytes({ whirlpool: { aToB: true } }),
        whirlpoolPoolState.toBuffer(),
      ],
      program.programId
    );
    ...
    const initialSource = (
      await getAccount(provider.connection, actualUserSourceTokenAccount)
    ).amount;
    const initialIntermediate = (
      await getAccount(provider.connection, actualUserIntermediateTokenAccount)
    ).amount;
    ...
    assert(
      finalIntermediate > initialIntermediate,
      "Intermediate balance should increase"
    );
