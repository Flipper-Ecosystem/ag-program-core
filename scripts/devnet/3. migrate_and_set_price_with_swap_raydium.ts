...
async function createRaydiumPoolWithCustomPrice() {
  try {
    ...
    // Sort tokens
    [sortedTokenAMint, sortedTokenBMint] =
    ...
    [poolState] = PublicKey.findProgramAddressSync(
      [
        ...
        sortedTokenAMint.toBuffer(),
        sortedTokenBMint.toBuffer(),
      ],
...
async function executeRouteSwap() {
  try {
    ...
    const remainingAccounts = [
      ...
      {
        pubkey: mockRaydiumProgram.programId,
        isWritable: false,
        isSigner: false,
      },
    ...
    const initialInput = (await getAccount(connection, userTokenAAccount))
      .amount;
    const initialOutput = (await getAccount(connection, userTokenBAccount))
      .amount;
    ...
    const finalOutput = (await getAccount(connection, userTokenBAccount))
      .amount;
    ...
    console.log(
      "   Exchange rate: 1 Token A =",
      (Number(received) / Number(spent)).toFixed(4),
      "Token B"
    );

    if (
      spent === BigInt(inAmount.toString()) &&
      received >= BigInt(minOutAmount.toString())
    ) {
