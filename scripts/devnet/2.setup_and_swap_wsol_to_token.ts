...
async function executeSwap() {
  try {
    console.log(
      "🔄 Executing single-hop swap with Raydium adapter (wSOL to custom token)...\n"
    );
    ...
    const initialSource = (await getAccount(connection, userSourceTokenAccount))
      .amount;
    const initialDest = (
    ...
    // Get final balances
    const finalSource = (await getAccount(connection, userSourceTokenAccount))
      .amount;
    const finalDest = (
      await getAccount(connection, userDestinationTokenAccount)
    ).amount;
    ...
    if (
      finalSource === initialSource - inAmountBN &&
