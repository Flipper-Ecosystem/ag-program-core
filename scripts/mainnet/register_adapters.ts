...
const WHIRLPOOL_PROGRAM_ID = new PublicKey(
  process.env.WHIRLPOOL_PROGRAM_ID ||
...
async function registerAdapters() {
  ...
  try {
    ...
    if (error.message.includes("Account does not exist")) {
      throw new Error(
        "Adapter registry does not exist. Please run initialize_adapter_registry.ts first."
      );
    }
    throw error;
  }
  ...
  try {
    ...
    console.error(
      "❌ Failed to register Whirlpool adapter (aToB: true):",
      error
    );
    throw error;
  }
  ...
  try {
    ...
    console.error(
      "❌ Failed to register Whirlpool adapter (aToB: false):",
      error
    );
    throw error;
  }
