...
const METEORA_LB_PAIR = new PublicKey("HTvjzsfX3yU6BUodCjZ5vZkUrAxMDTrBs3CJaq43ashR");
const METEORA_BIN_ARRAY_BITMAP_EXT = new PublicKey("9HcJeBEsq5px2bYZbdo7vzQWVsPK3SHTkchy42hBn7HC");
const METEORA_RESERVE_X = new PublicKey("H7j5NPopj3tQvDg4N8CxwtYciTn3e8AEV6wSVrxpyDUc");
const METEORA_RESERVE_Y = new PublicKey("HbYjRzx7teCxqW3unpXBEcNHhfVZvW2vW9MQ99TkizWt");
const METEORA_ORACLE = new PublicKey("EgEYXef2FCoEYLHJJW74dMbom1atLXo6KwPuA6mSATYA");
const METEORA_HOST_FEE_IN = new PublicKey("LBUZKhRxPF3XUpBCjp4YzTKgLccjZhTSDM9YuVaPwxo");
const METEORA_EVENT_AUTHORITY = new PublicKey("D1ZN9Wj1fRSUQfCjhvnu1hqDMT7hzjzBBpi12nVniYD6");
...
const PLATFORM_FEE_ACCOUNT = new PublicKey("BHbNdVGbHSxskzafynuc9p1zT8WM7sGiJUPV4q8szxQB");
...
async function main() {
    ...
    const [meteoraPoolInfo] = PublicKey.findProgramAddressSync(
        [Buffer.from("pool_info"), getSwapTypeBytes({ meteora: {} }), METEORA_LB_PAIR.toBuffer()],
        flipperProgram.programId
    );
    ...
    try {
        const platformFeeAccountInfo = await getAccount(connection, PLATFORM_FEE_ACCOUNT);
    ...
    try {
        ...
        // Get final balances
        const finalWsolBalance = (await getAccount(connection, userWsolAccount)).amount;
        const finalUsdcBalance = (await getAccount(connection, userUsdcAccount)).amount;
